...
代码迁入后使用 `pytest`，测试放在 `tests/`，文件命名为 `test_*.py`。优先覆盖消息解析、人工接管、意图路由、安全过滤、订单幂等、库存预占事务、API 发货失败、重新上架配置校验、商品归属校验和重新上架结果解析。Playwright 真实重新上架属于人工验收；单元测试只覆盖字段映射、状态解析和错误分支。
