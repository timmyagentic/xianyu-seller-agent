...
- `text`：固定文本。
- `data`：一次性库存行。必须先在 SQLite 事务中按订单和购买数量预占库存；数量为 N 时一次性预占 N 行，发送成功后标记 `sent`；发送失败时保留为 `reserved` 或 `failed_retryable`，由同一订单重试继续使用原预占行或人工释放。
...
5. 如果配置类型是 `data`，在同一个 SQLite 事务里按订单数量预占库存行；同一订单已有完整预占时复用，库存不足时不做部分预占，其他订单不能再读取已预占行。
...
当前绑定账号没有开通鱼小铺，重新上架设计必须以普通闲鱼账号为默认路径。`seller.goofish.com` 的卖家工作台/鱼小铺页面出现 `no-permission` 是预期现象，不能作为普通账号重新发布能力不足的判断依据，也不能阻塞自动发货、订单确认或商品同步链路。只有在后续明确切换为已开通鱼小铺的账号时，才把 seller 工作台作为默认执行入口。
...
6. 打开普通发布页的重新发布路由 `https://www.goofish.com/publish?itemId=...&editScene=rePutOn`，定位目标 `item_id` 或标题，检查登录状态、滑块验证和发布按钮；普通账号页面没有库存输入框时，只记录本地目标库存，不伪造平台库存修改成功。
...
- `delivery_inventory`：发货配置、库存内容、状态、预占订单、预占批次、预占行号、预占时间、发送时间、失败原因。库存行状态更新必须在事务中完成；同一订单多数量发货使用同一个 `reservation_id` 和递增 `reservation_line_no`，并对 `(reserved_order_no, reservation_line_no)` 建唯一约束，避免同一订单重复预占同一行号。
...
- 自动发货：订单解析、重复订单跳过、库存预占事务、多数量订单预占多行、并发订单不重复发同一库存行、发送失败可重试、API 失败不消费、发送失败日志。
